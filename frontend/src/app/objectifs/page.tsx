...
export default function ObjectifsPage() {
  const { objectifs, objectifsLoading, setObjectif, deleteObjectif, summary } = useFinancial()
      ...
      {objectifs.length === 0 ? (
        <div className="flex flex-col items-center justify-center h-80 gap-6 glass-card rounded-[40px] border border-dashed border-white/10">
          ...
          <div className="text-center space-y-2">
            <h3 className="text-lg font-bold text-white/60">Aucun objectif défini</h3>
            <p className="text-sm text-white/30 max-w-xs mx-auto">Commencez par créer votre premier objectif d'épargne pour visualiser votre progression.</p>
          </div>
          <button
