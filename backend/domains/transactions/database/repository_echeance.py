...
class EcheanceRepository:
    ...
    def migrate_from_recurrences(self) -> dict:
        """
        Migre les données de l'ancienne table 'recurrences' vers 'echeances'.
        ...
        try:
            logger.info("🚀 Début de la migration depuis 'recurrences'...")
            conn = get_db_connection(db_path=self.db_path)
            conn.row_factory = sqlite3.Row
            cursor = conn.cursor()
            ...
            if not cursor.fetchone():
                logger.warning("Table 'recurrences' introuvable. Migration annulée.")
                return stats

            cursor.execute("SELECT * FROM recurrences")
            recurrences = cursor.fetchall()
            logger.info(f"Trouvé {len(recurrences)} enregistrements dans 'recurrences'")
            ...
            for rec in recurrences:
                try:
                    ...
                    current = parse(rec_dict["date_debut"]).date()
                    if rec_dict["date_fin"]:
                        date_fin = parse(rec_dict["date_fin"]).date()
                    else:
                        date_fin = date(today.year + 1, 12, 31)

                    delta = FREQ_DELTAS.get(rec_dict["frequence"].lower())
                    if not delta:
                        logger.warning(f"Fréquence inconnue: {rec_dict['frequence']}")
                        stats["skipped"] += 1
                        continue
                    ...
                    while current <= date_fin:
                        cursor.execute(
                            ...
                            (nom, type, categorie, sous_categorie, montant, frequence,
                             date_prevue, date_debut, date_fin, description, statut,
                             type_echeance, recurrence_id, date_creation)
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'active', 'recurrente', ?, Datetime('now'))
                        """,
                            (
                                rec_dict.get("description", "Échéance"),
                                rec_dict["type"],
                                rec_dict["categorie"],
                                rec_dict.get("sous_categorie"),
                                rec_dict["montant"],
                                rec_dict["frequence"],
                                current.isoformat(),
                                rec_dict["date_debut"],
                                rec_dict.get("date_fin"),
                                rec_dict.get("description"),
                                str(rec_dict["id"]),
                            ),
                        )
                        stats["migrated"] += 1
